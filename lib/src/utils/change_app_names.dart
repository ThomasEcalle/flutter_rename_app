...
_changeAndroidValues(Config config) async {
  ...
  for (final FileSystemEntity resourceEntity in resourcesEntities) {
    ...
    if (resourceName.contains("values") && resourceEntity is Directory) {
      ...
      /// if lang is null, then we are in the default values directory and we skip it
      if (lang == null) continue;

      if (newI18nAppNames.containsKey(lang)) {
        await _changeNameInValueDirectory(newI18nAppNames, lang);
      } else {
        await _removeNameInValueDirectory(newI18nAppNames, lang);
      }
    }
  }
}

/// Write the new name in the strings.xml of the concerned locale
/// There is 3 case :
...
_changeNameInValueDirectory(Map<String, String> newI18nAppNames, String lang) async {
  // TODO: Handle strings.xml existence, <resources> existence, etc.

  final String filePath = "android/app/src/main/res/values-$lang/strings.xml";
  ...
  if (result != null) {
    await Utils.changeContentInFile(
      ...
      '<string name="app_name">${newI18nAppNames[lang]}</string>',
    );
  } else {
    final String stringsContent = File(filePath).readAsStringSync();
